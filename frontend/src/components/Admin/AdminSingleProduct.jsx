...
const SingleProduct = ({ product }) => {
  ...
  const formSubmitHandler = (e) => {
    e.preventDefault();
      try {
        console.log(formRef.current._id,formRef.current);
         dispatch(updateProduct(formRef.current._id,formRef.current));
         toast(
          {
            title: 'Product Updated',
            description: `${formRef.current.title} has been updated successfully.`,
            status: 'success',
            duration: 3000,
            isClosable: true,
          });
        ...
        toast(
          {
            ...
            description: `${formRef.current.title} has not edit.`,
            status: 'success',
            duration: 3000,
            isClosable: true,
          });
  ...
  const Form1 = () => {
    return (
      <>
        ...
        <FormControl mt={'2'}>
          <FormLabel htmlFor="name" fontWeight={'normal'}>Title</FormLabel>
    ...
    return (
      <>
        ...
        <SimpleGrid>
          <Flex>
            <FormControl mr="5%">
              <FormLabel htmlFor="originalPrice" fontWeight={'normal'}>Price</FormLabel>
              <Input id="original-price" name='price' onChange={formChangeHandler} fontSize={'small'} size={'sm'} />
            </FormControl>
          </Flex>
          <FormControl mt={'2'}>
  ...
  const handleDelete = (product) => {
    try {
      ...
      toast({
        ...
        description: `${product.title} has not deleted`,
        status: 'error',
        duration: 4000,
        isClosable: true,
      })
    ...
    <Center bg='white' mt='2'>
      <Box role={'group'} p={6} maxW={'200px'} w={'full'} boxShadow={'2xl'} rounded={'lg'} pos={'relative'} zIndex={1}>
        <Box
          ...
          <Image rounded={'lg'} boxSize={150} objectFit={'cover'} src={product.img1} />
        </Box>
        <Stack align={'center'}>
          <Heading fontSize={'md'} fontFamily={'body'} fontWeight={500}>{product.title}</Heading>
